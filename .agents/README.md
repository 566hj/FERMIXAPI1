...
FermixAPI / FermixCoin вместо текущего ассистента (Devin).
...
- Не переименовывать публичный API `FermixHint` / `FermixHintStack` /
  `FermixCore` — на него уже завязан плагин FermixCoin.
...
- Перед коммитом: `dotnet build -c Release` для FermixAPI.csproj и
  для `plugins/FermixCoin/FermixCoin.csproj`. Должно быть
  **0 errors, 0 warnings** в обоих.
- После создания PR — дождаться CI (4 проверки) и убедиться, что все
  зелёные.
